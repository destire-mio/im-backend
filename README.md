...
POST /messages
  ...
  -> Outbox Worker: claim
  -> PostgreSQL: 分配用户 cursor + 写入 sync event + 补全 outbox payload（同一事务）
  -> Redis / 本地 Hub
  -> WebSocket 实时通知
...
当前本机隔离库证据显示：3000 req/s、60000 条消息的 bulk projection 复核中，HTTP、Realtime 和 Sync 均完整，Realtime P95 为 0.762 秒；提高到 3500 req/s 后出现明确积压，不能宣称系统容量为 3500 req/s。原始报告分别见 [3000 req/s 复核](./loadtest-rate-3000-bulk-default.json) 和 [3500 req/s 诊断](./loadtest-rate-3500-bulk-default-p32.json)。
